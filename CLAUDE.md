...
| `documentation-master` | Any agent | Extraction: stages an existing project's *real* logic into a versioned Notion corpus — atomic claims, each with declared provenance (`codigo` / `entrevista` / `auditoria` / `historial`) and `file:line` evidence or an explicit `NO DETERMINADO`; owns the corpus contract (`interop-notion.md` §8 + `references/corpus.md`) and reports coverage per projection. Produces **no output files**: renderers project from the corpus |
