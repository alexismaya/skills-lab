...
Most skills operate over a shared Notion project: `project-audit` diagnoses an existing one, `derivar-proyecto` and `sdd-harness-notion` build it, `project-onboarding` documents its surface, `documentation-master` extracts the logic underneath it into a persistent corpus, `project-deck` presents it, and the QA skills test it. `git-workflow` is cross-cutting — it governs how agents use Git in any repo (pipeline project or not) and coordinates through the same Notion contract:
...
| `documentation-master` | Any agent | Extraction: stages an existing project's *real* logic into a versioned Notion corpus — atomic claims, each with declared provenance (`codigo` / `entrevista` / `auditoria` / `historial`) and `file:line` evidence or an explicit `NO DETERMINADO`; owns the corpus contract (`interop-notion.md` §8 + `references/corpus.md`) and reports coverage per projection. The `operacion` block (bring-up, environments, accesses, release criteria) is the one it fills by **directed interview**, never inferred from code; `riesgo` / `trayectoria` / `pruebas` come from other skills. Produces **no output files**: renderers project from the corpus |
...
| `project-doc` | Claude | Projection: renders `documentation-master`'s corpus into a `.docx` for one declared audience (user manual, training, PM doc, technical handover, client, performance attestation); a missing mandatory block blocks the section with its owner named — it is never filled by inference. Delegates `.docx` generation to the environment's `docx` skill |
